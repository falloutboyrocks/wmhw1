...
print "avg wordcount: " + str(avg_wordcount) ##############################
...
print "Done Reading Inverted List"
cursor = 0
test1 = False   #####################################
test2 = False   ####################################
...
for q in query_list:
	...
	if not word in bigram_record:
	    ...
	    for file_record in bigram_record[word]:
	        ...
	        file_score[file_record[0]] += file_record[1] * biIDF[word]
    sorted_list = sorted(file_score.items(), key=operator.itemgetter(1))
    sorted_list.reverse()
    for i in range(rank_num):
        f.write(fileID_docID[sorted_list[i][0]][0] + ' ')
        s.write(fileID_docID[sorted_list[i][0]][0] + ':' + str(sorted_list[i][1]) + '\n')
    f.write('\n')
    s.write('\n')
